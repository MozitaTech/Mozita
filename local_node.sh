...
update_test_genesis () {
    ...
    cat $HOMEDIR/config/genesis.json | jq "$1" > $HOMEDIR/config/tmp_genesis.json && mv $HOMEDIR/config/tmp_genesis.json $HOMEDIR/config/genesis.json
}
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# # validator address 0x3fB28E0eFB84EEAdf3A011c39558E2035FB0775e | evmos1fx944mzagwdhx0wz7k9tfztc8g3lkfk6rrgv6l | 894b7719cc6bf8698c57cec1efd5edfb964d8e3982bea7057806856c83ba09e7
	# USER5_KEY="validator"
	# USER5_MNEMONIC="pattern evoke erosion cement lonely cry captain power luxury bachelor delay athlete"
	...
	# echo "$USER5_MNEMONIC" | evmosd keys add "$USER5_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	...
	# Add aggregator address to list genesis account SAI
	# evmosd add-genesis-account "$(evmosd keys show "$USER5_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000aevmos --keyring-backend "$KEYRING" --home "$HOMEDIR"
	...
	# Add aggregator address to list genesis account
	# evmosd gentx "$USER5_KEY" 1000000000000000000000aevmos --gas-prices ${BASEFEE}aevmos --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
    ...
    update_test_genesis '.app_state["mint"]["params"]["mint_denom"]="aevmos"'
    update_test_genesis '.app_state["gov"]["params"]["min_deposit"]=[{"denom":"aevmos","amount": "1000000"}]'
    update_test_genesis '.app_state["gov"]["params"]["expedited_min_deposit"]=[{"denom":"aevmos","amount": "50000000"}]'
    update_test_genesis '.app_state["crisis"]["constant_fee"]={"denom":"aevmos","amount":"1000"}'
    update_test_genesis '.app_state["staking"]["params"]["bond_denom"]="aevmos"'

	# add centralized sequencer (validator)
	ADDRESS=$(jq -r '.address' $HOMEDIR/config/priv_validator_key.json)
	PUB_KEY=$(jq -r '.pub_key.value' $HOMEDIR/config/priv_validator_key.json)
	...
	# In giá trị của pubkey vào tệp output.txt
	echo "$PUB_KEY_VALUE" > output.txt
	echo "$ADDRESS" > output2.txt
fi
...
# Start the node
# --rollkit.da_address "0.0.0.0:$DA_GRPC" \
# evmosd start \
# 	--metrics "$TRACE" \
# 	--log_level $LOGLEVEL \
...
# 	--chain-id "$CHAINID" \
# 	--rollkit.aggregator \
# 	--rollkit.da_address=":46650" \
# 	--rollkit.da_start_height= "$DA_BLOCK_HEIGHT" \
# 	--rollkit.da_namespace "0000cd80e536ee2a398e" \
# 	2>&1 | tee outputEvmosd.log
evmosd start 
